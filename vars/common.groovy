def codeCheckout() {
    stage('Code Checkout') {
        sh 'sudo find . | sed 1d | xargs rm -rf'
...
def codeQuality() {
    stage('code Quality') {
        withCredentials([usernamePassword(credentialsId: 'SONAR', passwordVariable: 'sonarPass', usernameVariable: 'sonarUser')]) {
            sh '''
               sudo sonar-scanner -Dsonar.host.url=http://172.31.14.212:9000 -Dsonar.login=${sonarUser} -Dsonar.password=${sonarPass} -Dsonar.projectKey=${COMPONENT}  -Dsonar.qualitygate.wait=true
